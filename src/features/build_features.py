...
def encode_coupon_freq(df: pd.DataFrame, col: str = "coupon_frq") -> pd.DataFrame:
...
def encode_onehot(df: pd.DataFrame, col: str) -> pd.DataFrame:
    """We use sklearn module for one hot encoding"""
    logger.info(f"One Hot Encode column {col}")
    one_hot_encoder = OneHotEncoder(sparse=False, handle_unknown="ignore")
    df_enc = pd.DataFrame(
        one_hot_encoder.fit_transform(df[[col]]), columns=one_hot_encoder.categories_
    ).add_prefix(col + "_")
...
def encode_cfi(df: pd.DataFrame, col: str = "cfi_code") -> pd.DataFrame:
...
def country_spread(df) -> pd.DataFrame:
    ...
    df_pivot = pd.pivot(
        df,
        index=["rate_dt", "timeband", "actual_dt", "time"],
        columns=["country"],
        values=variables,
    )
    for variable in variables:
        for combi in itertools.combinations(countries, 2):
            ...
            df_pivot["cs_" + c0 + c1] = (
                df_pivot[variable, combi[0]] - df_pivot[variable, combi[1]]
            )
