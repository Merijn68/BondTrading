...
def join_price(df_bonds: pd.DataFrame, df_price: pd.DataFrame) -> pd.DataFrame:
    """join bonds and price data on isin = reference_identifier"""

    df_bonds = df_bonds.drop("ccy", axis=1)
    df = df_price.merge(
        df_bonds, left_on="reference_identifier", right_on="isin", how="left"
    )

    return df


def yield_to_maturity(df_bp: pd.DataFrame, df_yield: pd.DataFrame) -> pd.DataFrame:
    """Calculate for each bond per rate data the Yield to Maturity"""

    df_y = df_yield[["country", "rate_dt", "time", "mid"]].rename(
        columns={"mid": "ytm"}
    )
...
def join_yield(
    ...
    df_yield["offset"] = df_yield["timeband"].str.extract(r"(\d+)")

    # Join yield
    df_yield_pivot = pd.pivot(
        df_yield,
        index=["country", "rate_dt"],
        columns=["offset"],
        values=["bid", "offer"],
    )
    df_yield_pivot.columns = [
        "".join(("y_", "".join(tup))).strip() for tup in df_yield_pivot.columns
    ]
    ...
    df = df.merge(
        df_yield_pivot, left_on=["country", "rate_dt"], right_index=True, how="inner"
    )
...
def join_inflation(
    ...
    df = df.merge(
        df_inflation, left_on=["country", "rate_dt"], right_index=True, how="inner"
    )
...
def join_10y_inflation(
    df_bpy: pd.DataFrame, df_inflation: pd.DataFrame, country: str = "Germany"
) -> pd.DataFrame:
    ...
    if country == "":
        ...
        df_inflation = pd.pivot(
            df_inflation, index=["rate_dt"], columns=["timeband"], values="inflation"
        )
        df = df_bpy.merge(
            df_inflation, left_on=["rate_dt"], right_index=True, how="inner"
        )
